(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.SiraajAnalytics = {}));
    ...
    class AnalyticsCore {
      constructor(config = {}) {
        ...
        this.MAX_FAILED_QUEUE_SIZE = 50;
        this.MAX_STRING_LENGTH = 2048;
        this.RETRY_BASE_DELAY = 1e3;
        this.RETRY_MAX_DELAY = 32e3;
        this.STORAGE_PREFIX = "siraaj_analytics_";
        this.config = {
      ...
      track(eventName, properties = {}) {
        ...
        const event = {
          ...
          session_duration: Math.floor((Date.now() - (this.sessionStartTime || Date.now())) / 1e3),
      ...
      trackError(error, context = {}) {
        const errorData = typeof error === "string" ? { message: this.sanitizeString(error) } : {
      ...
      detectChannel(referrer, url) {
        const urlLower = url.toLowerCase();
        const referrerLower = referrer.toLowerCase();
        if (urlLower.includes("utm_medium=cpc") || urlLower.includes("utm_medium=ppc") || urlLower.includes("utm_medium=paid") || urlLower.includes("utm_source=paid") || urlLower.includes("gclid=") || urlLower.includes("fbclid=") || referrerLower.includes("/ads") || referrerLower.includes("adwords") || referrerLower.includes("googleads")) {
          return "Paid";
        }
      ...
      sanitizeProperties(props) {
        ...
        for (const [key, value] of Object.entries(props)) {
          ...
          if (typeof value === "string") {
            ...
            sanitized[sanitizedKey] = value.slice(0, 100).map(
